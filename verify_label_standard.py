...
class LabelStandardizationVerifier:
    ...
    def load_configuration(self):
        ...
        self.config = {
            ...
            "feature_branch": {
                "name": "main",
                "doc_file": "GIT-HUB/labels-standard.md"
            },
    ...
    def setup_environment(self):
        ...
        # 测试API连接 - 使用仓库基本信息端点
        test_success, _ = self.github_api_request("")  # 空字符串会自动处理为仓库基本信息
        if not test_success:
    ...
    def github_api_request(self, endpoint: str) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[API错误] {endpoint}: HTTP {response.status_code} - {response.text[:100]}", file=sys.stderr)
                return False, None
...
class LabelStandardizationVerifier:
    ...
    def load_configuration(self):
        ...
        self.config = {
            ...
            # 功能分支配置
            "feature_branch": {
                "name": "main/label-verification-project",
